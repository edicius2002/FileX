...
CELDAS = [
    ...
    ("png-caja-sin-pixeles", "png_tinta_cajas",
     ...
     '                               "tinta_pct": 0.0})',
     "PngTintaCajas"),
    # ---- _translate_acumulado (el arreglo de §5.1) -----------------------
    ("tr-suma-tx", "_translate_acumulado",
     "                tx += _num(piezas[0])",
     "                tx += 0.0",
     "SvgTextos"),
    ("tr-suma-ty", "_translate_acumulado",
     '                ty += _num(piezas[1]) if len(piezas) > 1 else 0.0',
     "                ty += 0.0",
     "SvgTextos"),
    ("tr-solo-translate", "_translate_acumulado",
     '                if nombre != "translate":\n                    return None',
     '                if False:\n                    return None',
     "SvgTextos"),
    ("tr-sube-al-padre", "_translate_acumulado",
     "        nodo = padres.get(id(nodo))",
     "        nodo = None",
     "SvgTextos"),
    ("tr-nomedible", "svg_textos",
     '        if despl is None:\n            # Un `transform` que no es una traslacion: la caja seria inventada.\n            r["n_no_medibles"] += 1\n            continue',
     '        if despl is None:\n            despl = (0.0, 0.0)',
     "SvgTextos"),
    # ---- png_tinta_cajas: el arreglo de §5.3 -----------------------------
    ("png-filtro-corrupto", "png_tinta_cajas",
     '                r["motivo"] = "PNG corrupto: %s" % e\n                return r',
     "                raise",
     "PngTintaCajas"),
    # ---- fidelidad_vectorial --------------------------------------------
    ("i9-nomedible", "fidelidad_vectorial",
     '        if tx["n_no_medibles"]:\n            cob["I9"] = False',
     '        if tx["n_no_medibles"]:\n            cob["I9"] = True',
     "FidelidadVectorial"),
