"""N16 — ¿hay alguna señal que separe el canal perdido a bitrates bajos?
...
def main() -> int:
    ...
    with open(os.path.join(AQUI, "a7_bitrate_bajo.json"), "w",
              encoding="utf-8") as fh:
        json.dump(res, fh, ensure_ascii=False, indent=1)
    os.makedirs(os.path.join(AQUI, "logs"), exist_ok=True)
    with open(os.path.join(AQUI, "logs", "a7_bitrate_bajo.log"), "w",
              encoding="utf-8") as fh:
        fh.write("\n".join(log) + "\n")
