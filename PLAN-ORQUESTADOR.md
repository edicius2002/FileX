...
- **`inspect` es la excepción a R8 y a R18**, y con número: en proceso cuesta **0,21–0,59 ms** *(corregido el 22/08 por `bench/hito4-mcp.md` §6.4: ~~0,04–0,06 ms~~ medía «abrir + leer 64 KiB de cabecera», no un `inspect`, que además clasifica el formato y recorre las cajas de un ISOBMFF)* frente a los **1,7–166 ms** del staging que R8 le impondría, **de 2,0× a 284× la operación a cambio de cero seguridad**. No entrega la ruta a ningún motor externo y no escribe nada (ídem §5.3).
...
| **La resolución de OCR la elige el ADAPTADOR DEL MOTOR: `ppp_ocr = max(min(nativos×1,25, techo), 100) × k(motor)`** *(el orden de `min` y `max` se corrigió el 22/08: escrito al revés, el techo anulaba el suelo para `n ≤ 80` y ordenaba 75 ppp sobre un original de 60 — **16,78 puntos**, `bench/corpus-d5.md` §2)* | **NO HAY UNA REGLA GLOBAL DE ppp — MEDIDO el 21/08 a las 14:00** (`bench/ppp-y-normalizacion.md` §2). Las dos versiones anteriores están refutadas: ~~×1,4 relativo~~ y ~~200 absoluto~~. **Los ppp no son la unidad** (el mismo mapa de bits en tres páginas da 19,13 / 19,63 / 36,24 % **a los mismos ppp** y coincide **a la centésima** a los mismos píxeles, 24 celdas), **y tampoco lo son ni un factor fijo ni una anchura fija**. **Siete configuraciones sobre el mismo documento dan óptimos entre ×0,50 y ×1,80**, y a ×1,4 sobre `d3` el mismo fichero es **seguro para PaddleOCR (3,80 %) y catastrófico para RapidOCR+R6 (46,84 %)**. **Suelo 100, con subida máxima ×1,25** (×1,4 solo es seguro en 4 de 8 parejas). **⚠ Y el 22/08 cambia CÓMO se fija `k`, no dónde vive** (`bench/k-por-motor.md`, 396 celdas): **el `k` óptimo es del PAR (motor, documento)** —motor 23,2 % de la varianza, documento 0,1 %, **interacción 76,7 %**, que es justo el término que la fórmula supone nulo—, y **el argmin de `escaneado_d4` es el mejor `k` fijo solo en 3 de 9 configuraciones**. Se fija por **mínimo arrepentimiento sobre varios documentos** (0,34–2,81 puntos en 9 de 9) y **se publica el arrepentimiento, no el CER del documento donde se midió**. Corregidos: **Docling+RapidOCR+R6 ×1,00 → ×0,875** (redondear cuesta 7,72 puntos en `d4c`) y **Tesseract ×1,50 → ×0,875 `psm 3` / ×0,75 `psm 11`** (el ×1,50 era el PEOR de los once factores: arrepentimiento 176,31 frente a 2,51). **Y dos variables pesan MÁS que el `k`: el `--psm` (42,78 puntos) y el RASTERIZADOR (33 puntos) — ningún `k` es comparable entre informes sin declarar los dos.** **Techo de coste: el tope interno del motor** (RapidOCR recorta a 2 000 px; por encima de 233 ppp recibe el array idéntico). Detalle en §4.5 |
...
- ~~**Tasks (SEP-1686) fue ELIMINADO de la especificación**~~ — **el HECHO se sostiene y la CAUSA se queda corta; corregido el 04/09/2026** (`bench/tasks-protocolo.md`, C48). **Tasks no se puede usar hoy, y por dos razones más fuertes que «lo quitaron de la especificación». (a) El SDK no trae el mecanismo:** en `mcp 2.0.0` no existen `mcp/{client,server,shared}/experimental`, `grep -rn "enable_tasks\|TaskStore"` da **0 líneas** y `dir(Server)` no tiene un solo atributo con `task`. **(b) El cliente no declara la capacidad:** Claude Code **2.1.260** anuncia `roots` y `elicitation`, y **`tasks` no está** — un `Task` nativo sería código muerto. **Lo que sí sigue existiendo son los TIPOS de cable, y sólo de la era `2025-11-25`:** 18 tipos de tarea en `_v2025_11_25` y **0** en `_v2026_07_28`. `mcp.types` reexporta el módulo **agregado** de las dos eras, así que `hasattr(types, 'CreateTaskResult')` da `True` y **no significa nada**: es la unión, no la era vigente. **Así que el `job_id` hay que construirlo entero:** `job_status` / `job_result` / `job_cancel`, con el vocabulario de estado de SEP-1686 (`working`/`completed`/`failed`/`cancelled`; el quinto, `input_required`, no aplica mientras FileX no elicite), un intervalo de sondeo sugerido por el servidor (~1.000 ms para conversiones) y un TTL. **Y no es una reimplementación evitable:** el mecanismo **se puede** rehacer a mano —los cuatro `tasks/*` se registran con `add_request_handler` y **responden de punta a punta, `rc=0`**—, pero el SDK **no anuncia** la capacidad y el cliente **no la pide**; y la mitad cara —persistir en disco, matar el árbol de procesos, TTL— hay que escribirla igual, porque la CLI, el watcher y la API **no hablan MCP**. **Coste medido de las tres salidas:** retirar `job` ahorra **153 tokens de catálogo (9,27 %)**; soportar los dos cuesta **+22 (+1,33 %)**; mantenerlo cuesta 0 y es lo recomendado. **Revisar cuando un cliente declare `tasks`** — la comprobación es una ejecución de ~7 s y está escrita en `bench/salidas-tasks-protocolo/`.
...
> | Resuelve 2 saltos **y explica por qué rechaza un camino que rasteriza** | ✅ **CERRADO el 22/08 con el motor documental** (`bench/hito5-documental.md`). `filex plan a.docx b.pdf` elige `docx→pdf` por LibreOffice y **descarta `docx→png→pdf` diciendo por qué**: *«rasteriza en `docx→png` y `pdf` admite texto: el resultado tendría la geometría correcta y ni una letra seleccionable»*. Es el **mismo binario** en las dos ramas —`--convert-to pdf` da 456 caracteres, `--convert-to png` da 0—, así que no compara motores sino caminos. **Y hubo que arreglar el núcleo para poder decirlo:** `TOPE_CANDIDATOS = 8` guardaba los ocho más baratos y un camino que rasteriza cuesta +1000, así que en cuanto hubo ocho alternativas buenas —con solo LibreOffice ya había siete— **el rechazo dejó de explicarse sin que ninguna prueba lo notara**. Ahora tiene hueco reservado: se explica, no se elige |
>
...
>
> **Deuda declarada:** (1) ~~`bench/scripts/verificador.py` se **importa** desde `bench/` en vez de vivir en `filex/` — moverlo ahora rompería las citas `fichero:línea` de doce informes y del patrón oro~~ **CERRADA el 22/08, y el motivo que la sostenía era FALSO: `git grep` sobre todo el árbol da CERO citas `verificador.py:NNN`** (`bench/hito3-mudanza.md`). Nunca existieron; lo di por hecho sin comprobarlo. La única referencia con número que hay —un traceback en `firmas-contrato.md`— **ya estaba caducada antes de mover nada**, porque el fichero creció 405 líneas después de aquel informe: **la trazabilidad la rompió el crecimiento normal, no el traslado**. El verificador vive ahora en `filex/verificador.py`, byte a byte, con los números de línea conservados, y `bench/scripts/verificador.py` es un envoltorio que aliasea en `sys.modules` para que los 19 arneses de `bench/` sigan viendo **un solo objeto-módulo**; (2) las aristas `sin_sondear` son 132 de 156: el sondeo real de capacidades por arista está **pendiente**, y hasta entonces cuestan +2,0 para no adelantar nunca a una medida; (3) `Ghostscript.orden` escribe **un solo fichero** y un PDF de varias páginas necesita `%d`, que es una salida multifichero y **el patrón oro no tiene ni una** (C22).
