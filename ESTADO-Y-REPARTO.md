...
>
> **Una fila = un identificador = exactamente un emoji de estado, y va en la última columna.** Verificado a máquina: **95 filas, 95 emojis.**
>
...
| **N12** | **La ventana entre la detección y el `move`.** `os.replace(p,p)` es un instante, no una vigilancia; se cierra abriendo el destino con `FILE_SHARE_NONE` vía `ctypes`. P no llegó: era lo último de su lista | 🔴 **NUEVO** · `cerrojo-de-maquina.md` §6.3, `cerrojo-unico.md` |
...
| **N15** | **La huella NO ve las TABLAS de datos, solo el código que las lee.** `EXT_FAMILIA` movió el `punto 1` de **3 de las 53 salidas del patrón oro** y **no caducó ni una arista**: el componente `contrato` no se movió. `huella.py` §2.8 declara que «no ve los DATOS» refiriéndose a `referencia.json` y al corpus; **tampoco ve las constantes de datos DENTRO del módulo que sí hashea**. Es la trampa 49 | 🔴 **NUEVO** · `contrato-familia-resvg.md` §6 |
| **N16** | **El punto ciego de A7**: por debajo de **48 kb/s** Opus rellena el canal mudo y la regla no dispara; a 32 kb/s falla por **1,03 dB**, y ese milímetro es la razón de no mover el umbral para atraparlo | 🔴 **NUEVO** · ídem §2.5 |
