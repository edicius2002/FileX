...
|---|---|---|
| 04/09 | **`bench/cancelacion-inestable.md`** (worker8, `cpu/cancelacion-inestable`) | **`N36` REFUTADA y cerrada: la inestabilidad es una fuga de huérfanos, no una cancelación frágil — y la fila nombraba a la prueba equivocada, que la escribí yo con n=6.** `test_cancelar_alcanza_al_motor_de_otro_proceso` da **0 fallos en 23 pasadas** y **16/16** aislada, la mitad con la máquina saturada a propósito. La inestable es `DuenoMuerto::test_sin_deteccion…`. **Mecanismo en dos piezas, separadas con número:** la condición necesaria es que `tearDown` mataba al **LANZADOR** (trampa 93) y el módulo dejaba **3 `ffmpeg.exe` codificando VP9 por pasada** (trampa 112); la causa próxima es que **`taskkill /F /T` no es atómico**, y el dueño sobrevive a su motor lo justo para escribir `failed` — así que **la condición que la prueba dice reproducir no se daba** (trampa 38). **A/B con carga: 3/8 → 8/8, huérfanos 2 → 0**; con el kill viejo pero sin la fuga, 6/6. **Arreglo de ARNÉS con `filex/` intacto —0 líneas de diff—, y el producto descartado con medida antes de decirlo.** Verificado aparte por el maestro: **5 de 5 pasadas limpias, 0 huérfanos tras cada una, y el módulo baja de 37-55 s a 26-28 s** — que la duración caiga a la mitad es lo que corrobora el mecanismo. Se refuta también su propia sonda: 20 de 20 verdes por una consulta CIM de ~1 s **antes** del kill. Trampa **123** |
| 04/09 | **`bench/aristas-reclasificacion.md`** (worker9, `cpu/aristas-reclasificacion`) | **`C49` cerrada: la sospecha del maestro es CIERTA y vale 0,90 puntos — el 54,78 % pasa al 53,88 %.** De los 445 `no_materializable`, **16 no son ficheros** (10 generadores de ImageMagick, 4 de `Module=URL`, 2 dispositivos de ffmpeg) y salen de la población: 138 501 → **135 535** aristas, indeterminadas 75 874 → **73 030**. **Los otros 429 están bien clasificados.** **El hallazgo grande está en el MOTIVO, no en el estado:** para **73 de los 445** `ffmpeg -muxers` declara que el binario **sí los escribe** —**12 242 aristas, el 16,13 % del estrato**— y **54 no se probaron nunca**, porque el censo de salida sólo cubrió los 202 destinos de ConvertX. **Cuatro de ellos —`dnxhd`, `dts`, `mlp`, `thd`— ya los había escrito `C28` 2/2**, en un informe hermano que ninguno de los dos podía cruzar. **Refuta 11 de las 26 conjeturas del encargo** —`hls`, `dash`, `rtp`, `rtsp` y `mpjpeg` son **muxers**, no protocolos, verificado aparte por el maestro— **y una propia**: su primer cruce dio 110 «desconocidos» y son **12**. Reproduce 138 501 / 40 252 / 22 235 / 75 874 / 140 **exactas antes de tocar nada** (trampa 58). **Consumo declarado: siete listados de metadatos**, cero conversiones y cero contenedores, con la máquina ocupada por otro agente. Trampa **122** |
| 04/09 | **`bench/uri-authority.md`** (worker7, `cpu/uri-authority`) | **`N37` cerrado: la *authority* de un `file://` se RECHAZA en vez de traducirse a UNC — y la fuga tenía una moneda peor de la que yo había medido.** No era sólo `file://servidor/recurso` → `\recurso`: **`file://nas-de-la-empresa/Work` concedía `D:\Work` ENTERO, con `D:\Work\research\ASR` dentro**, porque `abspath` completa con la unidad del proceso. **La decisión no la tomó la detección** —rechazar y traducir a UNC atrapan las mismas 4 fugas y rompen 0 legítimas—: la tomó **el precio**, porque admitir UNC mete un alias que `realpath` no colapsa y el cerrojo de destinos da entonces **dos dueños del mismo fichero justo cuando el destino no existe**, que es el caso normal de un conversor. Y el candidato «severo» —rechazar toda *authority*— **rompía 2 de 4 legítimas**: `localhost`, que RFC 8089 §2 iguala a la vacía y que **Node normaliza y Python no**, medido en los dos runtimes. **Sondeado en vivo y no deducido:** los dos productores reales emiten esa forma y el consumidor del mismo runtime la rechaza; Claude Code 2.1.260 responde con *authority* vacía. **Se refuta a sí mismo:** creyó cerrada la fuga del `cwd` y **no lo estaba** — `file:///D:` tiene unidad pero **no es absoluta**, y `abspath('D:')` devuelve el directorio actual de esa unidad (verificado por el maestro). Eran **tres** puertas al mismo `cwd` por tres mecanismos. Y se pilló **una trampa 111 en su propio informe**: la tabla de saltos venía de un `grep`, no de la traza. **A/B 10/10 con control de identidad, y el maestro lo reprodujo aparte: revertir `N34` da 3 rojas y revertir `N35` da 7** — los dos arreglos anteriores siguen vivos. Trampa **121** |
| 04/09 | **`bench/documentacion-publica.md`** (worker6, `orden/documentacion-publica`) | **La documentación, apta para un lector de fuera — y DOS puntos ciegos del propio comprobador.** `README.md` reescrito para que alguien que llega entienda en un minuto qué es FileX y qué demuestra; bloques de orientación en `HUECOS.md` y `ANALISIS-COMPLETO.md` —que **no declaraba** la convención MEDIDO/PENDIENTE y ahora dice que es anterior a ella en vez de fingir que la aplica—; aviso de instantánea fechada en `informe-filex.html`, nombrando las cuatro afirmaciones que el repositorio refutó después. **Once cifras caducadas corregidas**, entre ellas **`CONTRIBUTING.md` diciendo «las 107 trampas» cuando eran 119**. **Borró CERO ficheros**: el caso de `AGENTES-PRUEBAS-PENDIENTES.md` no era el documento —que ya lleva su aviso— sino el puntero del `README` que lo listaba como tarea viva; borrarlo habría dejado colgadas 6 referencias y destruido historia intelectual (trampa 106). **Y encontró una cita de commit muerta que `ci/integridad.py` no ve**, con su mecanismo verificado —murió al borrar las ramas `ccb/w*`, **no** en el `filter-repo`: no está en el `commit-map`—. **El maestro amplió el comprobador con su propuesta y salió una SEGUNDA muerta** (`1fb5024`, reparada con el `commit-map` a `dcd4057`), más el falso positivo que la ampliación generosa habría creado. Trampa **120** |
| 04/09 | **`bench/raices-mixtas.md`** (worker5, `nucleo/raices-mixtas`) | **`N35` cerrada PODANDO, y el reverso de la fuga de la ronda 15 queda cerrado SIN reabrirla.** Una raíz que no confina se descarta ella sola en vez de invalidar el conjunto: un cliente que declare `["C:\", <legítimo>]` conserva el legítimo. Elegido con **4 candidatos × 8 filas × 2 superficies** (núcleo y MCP), no con una intuición. **12 pruebas nuevas, y el maestro verificó la prueba causal en los DOS sentidos:** 10 celdas rojas contra el `confinamiento.py` anterior —en `test_hito1` **y** en `test_hito4`, que es lo que la trampa 26 exige— **y las tres pruebas de `N34` siguen siendo discriminantes**: pasan 3/3 con todo nuevo y **siguen rojas 3/3 con el `mcp.py` de antes de `N34`**, así que `N35` no las ha dejado vacuas (trampa 65). El camino de denegación **no paga nada** (trampa 28: el delta queda bajo el ruido y **el signo se invierte entre las tres tandas**). **Se refuta a sí mismo SEIS veces**, y la más importante no la vio él: una revisión independiente encontró que la frase que sostenía la decisión —*«la raíz de unidad es inerte, no concede nada»*— **era falsa** (concede 1 de 5: ella misma) **y que la prueba que la blindaba era vacua**. El veredicto aguanta porque los motivos buenos son estructurales —monotonía del OR, y que ese acceso nunca existió— **pero aguantó por suerte**. Trampas **118** y **119** |
| 04/09 | **`bench/roots-concurrencia.md`** (worker3, `cpu/roots-concurrencia`) | **`N34` cerrada SERIALIZANDO, y de paso DOS FUGAS DE SEGURIDAD del mismo `asegurar` que nadie buscaba.** N herramientas concurrentes con la caché fría pasan de **N idas y vueltas a 1** (8→1 con N=8), y el estado final deja de depender del orden de terminación: dos órdenes del mismo par daban `sin_acceso=False` **con** confinamiento y `sin_acceso=True` **sin** él. Elegido con una tabla de cuatro candidatos, no con una intuición (trampa 51). Cuesta **0,4 µs** en caliente —el camino caliente **no llega a tocar el candado**— y **0 ms de pared** (64,33→65,59 ms con N=8, dentro del ruido). **Las dos fugas: (a)** un `roots/list` fallido con `--raiz` puesta sellaba **la lista blanca del servidor entera**, más ancha que la intersección que exige R13; **(b)** el `except ValueError` convertía «ninguna raíz confina» (R3) en **«sin confinamiento» con la puerta abierta** — un cliente que declarase `C:\` leía ficheros de otro directorio **y de otra unidad**. El núcleo no se traga ese `ValueError`: **el agujero era exclusivo de la superficie MCP**. Verificado por el maestro con la prueba causal: sus 3 pruebas nuevas **fallan 3/3 contra el código anterior** y pasan 3/3 con el arreglo. Refuta a worker2 (*«las dos llamadas dan lo mismo»* es falso desde su propio arreglo de M3), reescribe una prueba suya de ayer sin relajarla, y **se refuta a sí mismo dos veces**. Trampa **116** |
| 04/09 | **`bench/tasks-protocolo.md`** (worker4, `cpu/tasks-protocolo`) | **`C48` cerrada: se mantiene el `job_id` propio, y el «no» es por INUTILIDAD, no por arquitectura.** `PLAN-ORQUESTADOR.md` §5.3 acierta el hecho y se queda corto en la causa (trampa 58): el mecanismo **no existe** en `mcp 2.0.0` —cero `enable_tasks`, cero `TaskStore`, cero módulos `experimental`— y lo que worker2 midió son los **tipos de cable de una era**: **18 en `_v2025_11_25` y 0 en `_v2026_07_28`**, reexportados desde el módulo agregado, reproducido por el maestro. Su *«ninguno está deprecado»* no prueba nada, con control positivo: el SDK **sí** marca `ServerSession.list_roots`. **El negativo que decide es del CLIENTE: Claude Code 2.1.260 —comprobado por el maestro— NO declara `tasks`**, así que un `Task` nativo sería código muerto. **Hecho nuevo:** el mecanismo **sí se puede rehacer a mano** (los cuatro `tasks/*` responden de punta a punta, `rc=0`), pero el SDK no lo anuncia y la mitad cara —persistencia, matar el árbol, TTL— hay que escribirla igual porque **CLI, watcher y API no hablan MCP**. Migrar ahorra **153 tokens (9,27 %)** y compra una superficie que **0 de 1 clientes medidos** puede usar. **Trampa 80 resuelta a favor:** sin línea nueva en `pyproject.toml`. Y se refuta a sí mismo: entró a refutar §5.3 y la sonda reprodujo el informe de hace dos semanas. Trampa **117** |
...
| **C5** | **MITAD CERRADA.** El cruce `inspect` vs staging: **`cruce_MB ≈ ffprobe_ms × copia_MBps / 1000`**, entre **~70 MB** (disco contendido) y **~95 MB** (holgado) — y de ahí sale que **`inspect` queda exento de R8 y de R18**. **La carrera de symlinks en Linux sigue BLOQUEADA**: el arnés (`c5a_symlink_wsl.py`) está listo, el vector está identificado y medido (swap dir-real/symlink, primitivo disponible en POSIX y bloqueado en Windows), y la **VM de WSL2 cae con `0x8007274c`** bajo contención. *«No es un resultado negativo: es una medición no hecha»*. **⚠ SU BLOQUEO ESTÁ CADUCADO — MEDIDO el 04/09/2026 por el maestro.** El `0x8007274c` se midió el **22/08** sobre una VM de **2 vCPU y 1,9 GiB**; el `.wslconfig` del usuario se modificó el **25/08** —tres días después— y hoy declara **`memory=10GB`, `processors=6`, `swap=2GB`**, es decir ×5 de memoria y ×3 de núcleos. **Y WSL2 responde ahora mismo**: `wsl -e uname -a` devuelve con `rc=0`. **No se afirma que la carrera de symlinks vaya a medirse sin problemas** —lo comprobado es un `uname`, no el arnés bajo contención—, sólo que **«bloqueada» ya no lo sostiene ninguna medida vigente**. Es la **trampa 95** en su forma exacta: *un rojo se investiga; un bloqueo se acepta*, y esta fila llevaba trece días aceptada sin que nadie mirase si su causa seguía viva. **El arnés `bench/salidas-mcp-cabos-2/c5a_symlink_wsl.py` está escrito desde el 21/08.** Deja de contarse como bloqueada: es **trabajo despachable** | 🟡 **EN CURSO — bloqueo caducado, despachable** · `mcp-cabos-2.md` §5.1 |
| **C6** | Replicar la saturación en **dominio documental** (docling-mcp, 19 herramientas), con API y `temperature` fija **CERRADA POR DECISIÓN DEL USUARIO el 04/09/2026: se descarta.** No va a haber clave de API. El motivo del bloqueo estaba bien identificado y es del INSTRUMENTO, no del sujeto —*«el CLI de Claude Code no expone `temperature` y no se puede fijar»*, que su propio informe llama **la limitación más seria del instrumento**— y sigue **escrito y vigente** en `saturacion-herramientas.md` §8 como limitación declarada de todas las medidas de saturación: **cerrar la fila no borra la salvedad**. Se cierra igual que `B4` y `B5`: un «no se pudo medir» documentado **es** un resultado, y mantener abierta una fila que nadie va a poder mover es lo que este documento ya pagó una vez. **Lo que queda sin medir, dicho para que nadie lo dé por hecho:** la saturación con `temperature` fija, en dominio documental (docling-mcp, 19 herramientas), y en modelos de otras familias o pequeños locales | 🟢 **CERRADO, descartado por decisión** · `saturacion-herramientas.md` §8 |
| **C7** | Si esas conversiones **se piden de verdad**. El catálogo de SnapOtter es un proxy de demanda **No es un bloqueo técnico: es que FileX no tiene usuarios todavía.** El catálogo de SnapOtter es un proxy de demanda, no demanda. **DECISIÓN DEL USUARIO, 04/09/2026: se APLAZA a producción** — se medirá con uso real, y **nadie la mide antes**. Queda explícitamente fuera del reparto: ninguna ronda la va a cerrar, y aproximarla con otra fuente (incidencias, foros, registros de un despliegue ajeno) **sería una fila distinta**, no ésta | 🔴 **ABIERTO, aplazado a producción por decisión** · `HUECOS.md` §2 |
...
| **C48** | **`PLAN-ORQUESTADOR.md` §5.3 da `Tasks` por eliminado del protocolo y `mcp 2.0.0` lo trae entero y sin deprecar — MEDIDO el 04/09/2026 por worker2** (`bench/mcp-cabos-y-techos.md` §8, sondeado ejecutando el SDK, no leyendo la especificación). Importa porque FileX ya tiene trabajos largos con `job_id` propio: si el protocolo trae el mecanismo, **el nuestro puede ser una reimplementación**. **PENDIENTE: medir si un `convert` largo se puede entregar como `Task` nativo y qué costaría en catálogo** — y hasta entonces **no se toca nada**, porque la versión que Claude Code negocia hoy es `2025-11-25`. **CERRADA el 04/09/2026 por worker4** (`bench/tasks-protocolo.md`): **se mantiene el `job_id` propio.** El mecanismo no existe en `mcp 2.0.0` y Claude Code **2.1.260** no declara la capacidad, así que migrar ahorraría **153 tokens de catálogo (9,27 %)** y compraría una superficie que **0 de 1 clientes medidos** puede usar; soportar los dos cuesta **+22 (+1,33 %)**. La mitad cara hay que escribirla igual porque **CLI, watcher y API no hablan MCP**. `PLAN-ORQUESTADOR.md` §5.3 queda corregido: acertaba el hecho y se quedaba corto en la causa. **Revisar cuando un cliente declare `tasks`** — la comprobación son ~7 s y está escrita | 🟢 **CERRADO** · `bench/tasks-protocolo.md` |
| **C49** | **¿Cuántos de los 445 `no_materializable` NO son ficheros? — CERRADA el 04/09/2026 por worker9** (`bench/aristas-reclasificacion.md`), y la respuesta es **16**, no los ~33 que el maestro conjeturó. Clasificados en **9 clases derivadas de los metadatos que publican los motores** —`magick -list format`/`-list delegate` y las cuatro listas separadas de ffmpeg— **nunca de los nombres**: 174 `solo_demuxer`, 98 `extension_de_demuxer`, 73 `declarado_muxer`, 65 `im_formato_real_solo_lectura`, 12 `desconocido_por_el_binario`, 10 `generador`, 7 `im_pseudo_operador`, 4 `protocolo`, 2 `dispositivo`. Retirando los 16, **138 501 → 135 535** y el 54,78 % → **53,88 %**. **La sospecha era cierta y pequeña**, y decirlo así vale más que el titular que se buscaba | 🟢 **CERRADO** · `bench/aristas-reclasificacion.md` |
| **C50** | **73 de los 445 llevan un motivo FALSO, y 54 no se probaron nunca — MEDIDO el 04/09/2026 por worker9** (ídem §4). Las 445 comparten **un solo string** de motivo, y para 73 `ffmpeg -muxers` dice lo contrario: el binario **sí los escribe**. Son **12 242 aristas, el 16,13 % del estrato indeterminado**, y **54 nunca se intentaron** porque `materializa()` sólo prueba con ffmpeg lo que está en el censo de **salida**, que cubrió los 202 destinos de ConvertX. **Esto NO necesita el corpus FATE de ~1 GB que reclama el pendiente de `aristas-nominales.md`**: son segundos de invocaciones de ffmpeg. **PENDIENTE, y pide máquina:** (1) escribir esos **54**, hasta **10 908 aristas**; (2) `ffmpeg -devices` **dentro del contenedor** para confirmar los 10 dispositivos de Linux (`alsa`, `x11grab`, `pulse`…) que worker9 **no movió a propósito**, porque nombrarlos por el nombre es el error que su informe vino a corregir — valdrían otras **~2 028**; (3) `clip` y `mask`, que ImageMagick declara `rw+` | 🔴 **ABIERTO** · `bench/aristas-reclasificacion.md` §4, §9 |
...
| **N34** | **La caché de raíces de `filex/mcp.py` NO es segura en la primera llamada concurrente — MEDIDO el 04/09/2026 por worker2** (`bench/mcp-cabos-y-techos.md` §2.4). Dos herramientas MCP que entran a la vez con la caché fría producen **2 `roots/list`**, no 1: el `_resuelto` se lee y se escribe con el lock, pero **la petición de en medio ocurre fuera de él**. Hoy no rompe nada —las dos respuestas son iguales— pero **el fallo transitorio que arregló M3 sí las hace distintas**, y entonces una carrera decide si la sesión queda con acceso o sin él. Lo destapó un contador dentro del doble, no la prueba (trampa 114). **PENDIENTE: decidir si se serializa la petición o se acepta la carrera con su motivo escrito** **CERRADA el 04/09/2026 por worker3** (`bench/roots-concurrencia.md`): se **serializa** —candado asíncrono sostenido a través del `await` **más** no sellar ningún resultado nacido de un fallo—, elegido con una tabla de cuatro candidatos. N=8 pasa de 8 idas y vueltas a **1**, cuesta **0,4 µs** en caliente y **0 ms de pared**, y **no empeora el caso del cliente mudo**. **Y destapó dos fugas de seguridad que no son de concurrencia**: un sellado más ancho que la intersección de R13, y un `except ValueError` que dejaba a FileX **leyendo otra unidad sin confinamiento**. Prueba causal verificada por el maestro: 3/3 rojas contra el código anterior. PENDIENTE declarado: la latencia real por el cable, la atomicidad frente a **hilos** (no corrutinas) y las raíces mixtas, que van a `N35` | 🟢 **CERRADO** · `bench/roots-concurrencia.md` |
| **N35** | **Las raíces MIXTAS deniegan de más — MEDIDO el 04/09/2026 por worker3** (`bench/roots-concurrencia.md` §4.3, celda N8 ya escrita). Cuando unas raíces confinan y otras no, `Confinamiento` lanza `ValueError` por el conjunto entero en vez de quedarse con las que sí confinan, así que la sesión pierde acceso a raíces legítimas. **Es el reverso exacto de la fuga que cerró `N34`** —aquella abría de más, ésta cierra de más— y por eso sale ahora: el mismo `except` las tapaba a las dos. **El arreglo NO es de la superficie MCP: vive en `filex/confinamiento.py`**, que no era de worker3 en la ronda 15. **PENDIENTE: decidir si el conjunto mixto se poda o se rechaza, y medirlo — la celda de prueba ya existe**. **CERRADA el 04/09/2026 por worker5** (`bench/raices-mixtas.md`): **se PODA** —las raíces que no confinan se descartan una a una y el `ValueError` queda para lo que R6 siempre quiso decir, *no queda ninguna*—, elegido con 4 candidatos sobre 2 superficies, más una guarda nueva que separa «no declaré escritura» de «declaré escritura y ninguna confina». **No reabre la fuga de la ronda 15: 0 accesos indebidos ganados sobre 11 filas, la celda de N7 SIN_CAMBIO, y verificado aparte por el maestro que las pruebas de `N34` siguen rojas contra el código de antes.** Y **corrige la lectura que todos hacíamos de R3**: una raíz de unidad no confina demasiado —la barra doble deniega hasta lo que está literalmente debajo— pero **tampoco «nada»**: concede la propia raíz. Lo que convierte «podar» de apuesta en teorema no es esa inercia sino la **monotonía del OR** de `_dentro`, más que ese acceso **nunca pudo existir** antes. PENDIENTE declarado, y ninguno es `N35`: `_uri_a_ruta` rompe los roots UNC canónicos, la raíz `""` confina en el `cwd`, y la poda es silenciosa | 🟢 **CERRADO** · `bench/raices-mixtas.md` |
| **N36** | **`test_cancelar_alcanza_al_motor_de_otro_proceso` es INESTABLE dentro de su módulo, y la trampa 101 ya no lo describe — MEDIDO el 04/09/2026 por el maestro al verificar la ronda 16.** La trampa 101(b) dejó escrito *«15 passed en tres pasadas seguidas al repetirla con la máquina tranquila»*; hoy **no reproduce**. Con el código de worker5: **1 de 3 pasadas limpia**. Con el `filex/` de `main` como control: **2 de 3**. **Aislada, la prueba pasa 3 de 3** —así que no es la prueba ni la máquina: es una interacción DENTRO del módulo, que es la forma de la trampa 103—. **El cambio de la ronda 16 queda descartado como causa**: falla igual sin él, y `git diff` no toca nada de cancelación. Las pasadas que fallan son las lentas (53-55 s) y las que pasan las rápidas (37-39 s), pero **eso es una correlación de n=6, no un mecanismo, y no se publica como causa** (trampa 111). **PENDIENTE: reproducir con el orden de ejecución declarado y aislar el par de pruebas que interactúa**. **REFUTADA Y CERRADA el 04/09/2026 por worker8** (`bench/cancelacion-inestable.md`): **la prueba que esta fila nombraba NO es la inestable**, y el enunciado era del maestro, escrito con n=6 — con **n=23 da 0 fallos**, y **16/16** aislada con la máquina saturada a propósito. La inestable es `DuenoMuerto::test_sin_deteccion…`. El mecanismo es una **fuga de 3 `ffmpeg` por pasada** —`tearDown` mataba al **lanzador**, trampa 93— que fabrica la carga sobre la que se abre la ventana de un **`taskkill /F /T` no atómico**, y entonces **la condición que la prueba dice reproducir no se da** (trampa 38). **A/B 3/8 → 8/8, huérfanos 2 → 0**, y con el kill viejo pero sin la fuga, 6/6: las dos piezas separadas con número. **Arreglo de ARNÉS, `filex/` intacto (0 líneas).** Verificado aparte por el maestro: **5/5 limpias, 0 huérfanos, y el módulo baja de 37-55 s a 26-28 s**. **Y explica por qué la pregunta estaba mal planteada:** *«qué prueba falla»* no tiene respuesta estable porque el sujeto es el **módulo** — el «3 de 3 aislada» que midió el maestro era **verdadero y engañoso a la vez** | 🟢 **CERRADO** · `bench/cancelacion-inestable.md` |
| **N38** | **`test_watcher_n.py::BarridoDeHuerfanos::test_sin_candado_y_joven_no_se_toca` falló UNA vez en `windows-latest` — OBSERVADO el 04/09/2026 por el maestro, y n=1.** `assertFalse(os.path.isdir(d))` tras un barrido con `edad_sin_candado=0.0`: el directorio seguía ahí. **Historial: 11 éxitos antes y el relanzamiento del MISMO sha pasa**, así que es intermitente y no una regresión — el merge que lo precedió **no toca `filex/` ni ese fichero** (`git diff` vacío, el control que exige la trampa 101). En esta máquina da **6 de 6 limpias**, y eso **no dice nada de `windows-latest`** (trampa 104: la aptitud de un entorno se mide EN ese entorno). **No hay mecanismo y no se propone uno** (trampa 111): esto es una observación de n=1 que se registra **para que no se pierda**, que es justo el defecto que `N37` destapó hoy —un pendiente escrito dentro de un informe no existe para el reparto—. Llama la atención que sea **de la misma familia que `N36`**, cerrada horas antes: una prueba de **barrido/limpieza** sensible al reloj en Windows. **PENDIENTE: si vuelve a aparecer, contar cuántas de cuántas antes de tocar nada**. **VUELVE A APARECER el mismo día, y sube a n=2 con una prueba mucho más fuerte: la segunda vez fue sobre un PR cuyo ÚNICO cambio es una línea de markdown** (`33911467528`, misma prueba, misma aserción). **Eso descarta el código como causa sin necesidad de más control**: un `.md` no puede mover un barrido de directorios. Sigue sin proponerse mecanismo. **TASA MEDIDA el 04/09 por el maestro, que es quien tiene credenciales de `gh`** —un worker no puede lanzar ejecuciones de CI (`CONTRIBUTING.md` §7)—: **2 fallos en 47 ejecuciones del día, 4,3 %**, y **10 lanzamientos EN SERIE sobre `961b91c` dan 10 limpias**. **Eso NO lo despeja, y el número lo dice: `P(0 fallos en 10) = 0,65` con esa tasa**, así que ver diez verdes seguidas era más probable que no aunque el fallo sea real. **Un primer intento de medirlo se saboteó solo**: diez lanzamientos seguidos dieron **9 `cancelled`** porque el flujo lleva `concurrency: cancel-in-progress`, así que hay que lanzarlos en serie. Y la API **subcuenta**: devuelve `failure: 1` donde yo vi 2, porque relanzar reescribe la conclusión (trampa **125**). **PENDIENTE: para distinguir 4,3 % de 0 % hacen falta ~70 pasadas limpias seguidas; decidir si vale la pena o si se deja declarado** | 🔴 **ABIERTO, tasa 4,3 % (2/47)** · ejecuciones `33911108897` y `33911467528` |
| **N37** | **`_uri_a_ruta` DESCARTA la *authority* de un `file://` canónico, así que una raíz UNC declarada confina en una ruta LOCAL que nadie declaró — MEDIDO el 04/09/2026 por el maestro**, verificando un `PENDIENTE` que worker5 dejó escrito en `bench/raices-mixtas.md` y que **nadie había registrado**. Ejecutado sobre `filex.mcp._uri_a_ruta`: **`file://servidor/recurso` da `\recurso`** —el `servidor` desaparece— mientras `file://///servidor/recurso` sí conserva la forma UNC; y **la cadena vacía se queda vacía, que `abspath` resuelve al `cwd`**. Es la **misma familia** que las dos fugas que cerraron `N34` y `N35` —lo declarado y lo efectivo no coinciden en la lista blanca— y por eso importa: aquí la raíz no se pierde ni se poda, **se sustituye por otra**. RFC 8089 da la forma con *authority* como canónica para UNC, así que no es un caso rebuscado. **PENDIENTE: decidir si se rechaza el URI con *authority* no vacía o se traduce a UNC, y medir qué clientes la emiten** — la cadena vacía y el carácter silencioso de la poda van con ella. **CERRADO el 04/09/2026 por worker7** (`bench/uri-authority.md`): **se RECHAZA la *authority* de red** —salvo `localhost`, que RFC 8089 §2 iguala a la vacía— **y no se traduce a UNC**. La tabla de candidatos empata en detección (4 fugas de 4, 0 legítimas rotas) y **desempata el precio**: la traducción mete un alias que `realpath` no colapsa y el cerrojo de destinos daría **dos dueños del mismo fichero cuando el destino aún no existe**. **Cerradas también las otras dos**: la raíz vacía y —hallazgo suyo— **`file:///D:`, que tiene unidad pero no es absoluta**, así que eran **tres** puertas al `cwd` y no una; y la poda muda de `N35` deja de serlo (`_podadas`). Tocó `filex/confinamiento.py` de forma aditiva y lo declaró | 🟢 **CERRADO** · `bench/uri-authority.md` |
...
| **15** | *(sin carril GPU: **no queda ni una fila que pida la tarjeta**. `B3` era la última y se cerró ayer)* | **N34** (worker3, `cpu/roots-concurrencia`) y **C48** (worker4, `cpu/tasks-protocolo`) · %s **DESPACHADA el 04/09** — `bench/roots-concurrencia.md`, `bench/tasks-protocolo.md` | **El reparto ya no es por RECURSO, es por FICHERO**, y por primera vez hay que decirlo: las dos filas son del carril CPU, así que la exclusión estructural de `CONTRIBUTING.md` §2 no las separa. **worker3 POSEE `filex/mcp.py` y `pruebas/test_hito4.py`; worker4 no toca `filex/` en absoluto** —su propio hallazgo lo pide, porque Claude Code negocia hoy `2025-11-25`—. Las dos salen de la ronda 14, y las dos son deuda que **el trabajo destapó al cerrar otra cosa**. **CERRADA el 04/09**: las dos entregadas, y **el reparto por fichero funcionó** —los diffs no se solapan en una sola línea—. La ronda deja **dos fugas de seguridad cerradas que nadie había encargado**, y las dos aparecieron dentro del mismo `except` que se fue a tocar por un motivo de concurrencia |
| **16** | *(sin carril GPU, como la 15)* | **N35** (worker5, `nucleo/raices-mixtas`) · %s **DESPACHADA el 04/09** — `bench/raices-mixtas.md` | **UN SOLO worker, y se dice por qué:** de las seis filas vivas, tres están bloqueadas fuera de esta máquina (`C6`, `C7`, `C5`) y dos están en su techo (`C28`, `C36`), así que **`N35` es la única despachable** — con dos no habría trabajo honesto que repartir. Y la rama es **`nucleo/`** y no `cpu/`: el arreglo vive en `filex/confinamiento.py`, que **no es de ningún carril** y lo ven las cuatro superficies; `CONTRIBUTING.md` §2 pide que esos cambios *«se acuerden antes»*, y el acuerdo es que **nadie más trabaja esta ronda**. El encargo lleva su propio riesgo escrito: `N35` es **el reverso exacto** de la fuga que cerró `N34`, y **arreglar «deniega de más» sin reabrir «abre de más» es todo el trabajo**. **CERRADA el 04/09**: se podó, y la no-reapertura está verificada **por dos vías independientes** —la sonda del worker (11 filas, 0 accesos indebidos ganados) y la comprobación del maestro de que las pruebas de `N34` **siguen rojas** contra el código de antes—. La ronda deja **dos autorrefutaciones de las buenas** (trampas 118 y 119) y una fila nueva que **no trajo el worker sino la verificación** |
| **17** | *(sin carril GPU)* | **N37** (worker7, `cpu/uri-authority`) · 🟢 **ENTREGADA Y VERIFICADA** — `bench/uri-authority.md` | **Registrada al cerrarla, no al despacharla, y se dice**: se lanzó mientras la pasada de documentación corría y no hubo commit de registro entre medias. **Tercera ronda seguida sobre la lista blanca**, y por eso el encargo llevaba dentro su propio riesgo: *arreglar sin deshacer `N34` ni `N35`*. El A/B lo demuestra por dos vías —la del worker (10/10 con control de identidad) y la del maestro (3 y 7 rojas al revertir cada arreglo)—. En paralelo corrió **worker6** con la documentación pública, que no comparte un solo fichero con ésta |
| **18** | *(sin carril GPU)* | **N36** (worker8, `cpu/cancelacion-inestable`) · 🟢 **ENTREGADA Y VERIFICADA** | **La máquina EN EXCLUSIVA fue el recurso repartido**, por primera vez: el sujeto era la carga, así que el maestro se comprometió a no correr nada pesado y worker9 fue acotado a listados de metadatos. Diseño de dosis-respuesta (carga 0, 8, 12) que el encargo no pedía |
| **19** | *(sin carril GPU)* | **C49** (worker9, `cpu/aristas-reclasificacion`) · 🟢 **ENTREGADA Y VERIFICADA** | **Corrió EN PARALELO con la 18 sin estorbarla**, y ésa era la condición: análisis puro, siete listados de metadatos, cero conversiones. **Refutó al maestro** (15 aciertos de 26) y abrió `C50`, que vale más que el encargo |
...
| Ruta | Para qué |
|---|---|
| `CLAUDE.md` | Las reglas de trabajo, las **125** trampas ya pagadas y las reglas de diseño no negociables. ~~24~~ — **la cifra llevaba setenta y ocho trampas desfasada**, que es el mismo defecto que esta sección viene a arreglar |
