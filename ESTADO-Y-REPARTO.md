...
>
> **Una fila = un identificador = exactamente un emoji de estado, y va en la última columna.** Verificado a máquina: **103 filas, 103 emojis.**
>
> | Emoji | Significa | Cuántos hoy |
> |---|---|---|
> | 🟢 | **CERRADO** con informe que lo prueba | **62** |
...
| **N24** | **El contrato no tiene regla de bitrate de VÍDEO.** `verificador.py` solo compara bitrate contra pistas de **audio**, así que da `ok` a las 8 celdas de NVENC cuyo desvío va de **+9,82 % a +24,59 %**. El punto 4 del contrato —pedido frente a obtenido— no se aplica al vídeo | 🔴 **NUEVO** · `hito2-nvenc.md` §4 |
| **N25** | **El lock de GPU no rodea al CODIFICADO, solo a la sonda.** Falta una línea en `nucleo.py` que era de otro reparto; el parche exacto y su coste (**1 403,6 µs, 0,19 %**) están en §6.5 del informe | 🔴 **NUEVO** · ídem |
