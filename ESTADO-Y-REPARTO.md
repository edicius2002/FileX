...
| **A5** | **Commit. SIGUE ABIERTO, y es lo más urgente del inventario** — no se ha ejecutado ni `git add` ni `git commit`. **Van SIETE agentes sin commit y `git status` tiene 44 entradas.** La lista vigente está en `bench/consolidacion-3-21ago.md` §5, que actualiza la de `consolidacion-2-21ago.md` §5. **Recuerda: `corpus/pdf/escaneado_d4*.pdf` van por Git LFS** | 🔴 **ABIERTO** |
| **A7** | **Decidir si la métrica de OCR canónica del proyecto pasa a ser la acentuada.** Hoy `bench/scripts/ocr_eval.py` sigue intacto y sigue siendo ciego; hay **dos copias** con acentos (`ocr_eval_d4.py`, `ocr_eval_tildes.py`) y ninguna es la oficial | 🔴 **ABIERTO** |
...
| **B3** | **marker** — instalado y sin medir. `torch 2.13.0` **sin paquetes `nvidia-*`**: es build **CPU** | 🔴 agente B abandonado. **Sin lanzar** |
| **B4** | **surya** por `SURYA_INFERENCE_BACKEND=llamacpp` o `VLLM_GPU_MEMORY_UTILIZATION=0.5` | 🔴 agente C nunca lanzado |
| **B5** | **MinerU `[vlm]`** (no `[vllm]`) | 🔴 agente D nunca lanzado |
| **B6** | **NVENC en lote sobre carpeta real** — el único pendiente del hueco 4, y el único caso donde decide algo | 🔴 `HUECOS.md` §4 |
...
| **B8** | R1 sobre PDF que **no** son «una imagen a página completa»; e interacción de **`magick -deskew 40%`** con el techo, ahora sobre la familia d4 (rotada de −4° a +4°) | 🔴 ídem |
...
| **B11** | **Llevar la corrección a producción — y su contenido CAMBIA.** No es «añadir R6 a `bench/scripts/ocr_motor.py`»: **sobre el `PP-OCRv5 mobile` que usa hoy, R6 NO es recomendable** (4 de 15 celdas peores). Es **cambiar a `PP-OCRv6 small` Y añadir R6**. **Saldo medido, declarado entero: 7 mejor, 2 igual, 2 PEOR** (`d4a` +5,87 y `d4f` +1,01, por el cambio de checkpoint, no por R6). El parche exacto está en `ppp-y-normalizacion.md` §4, **propuesto y NO aplicado** | 🟡 **REDEFINIDO** · `ppp-y-normalizacion.md` §3.5, §4 |
| **B13** | **Caracterizar el `k` de cada motor sobre más de un documento.** Que *haya* un `k` por motor está apoyado por tres documentos; **el VALOR de cada uno es una estimación de un punto** (los siete óptimos salen de `escaneado_d4`). Hay que barrer al menos `d3`, `d4c` y `patologico` con las siete configuraciones **antes de cablearlo en el adaptador**. **Es el pendiente de más valor que abre P1** | 🔴 **NUEVO** · ídem §8 |
| **B14** | **Barrer la curva de ppp de Tesseract.** `escaneado_d2` refuta la regla con n=1 (**0,00 % a 150 ppp frente a 32,10 % a sus 100 nativos**) y `d4` va en la dirección contraria. **Ni siquiera dentro de un motor hay un factor único** | 🔴 **NUEVO** · `invocacion-aristas.md` §9, §11.6 |
| **B15** | **Un `escaneado_d5` de 60-80 ppp nativos.** El suelo de 100 ppp **nunca se ha probado con un original que lo necesite**: todo el corpus está entre 100 y 240, y el ×1,25 propuesto sale de acotar la subida en documentos que no la piden | 🔴 **NUEVO** · `ppp-y-normalizacion.md` §8 |
| **B16** | **Refinar `escaneado_d3` entre ×1,25 y ×1,4**, que es donde está el acantilado de RapidOCR+R6 (2,53 → 46,84) y solo hay dos puntos. **Es el mismo defecto que B9 vino a corregir en `d4`, en otro documento** | 🔴 **NUEVO** · ídem |
| **B12** | **Degradación realista en el corpus**: sombra de encuadernación, curvatura de página, transparencia del papel. `d4` es sintético | 🔴 **NUEVO** · ídem §5.3 |
...
| ~~**C8**~~ | Los 7 casos `no_evaluable` de `referencia.json` | ✅ **5 de 7 CERRADOS** por E1 dentro del contenedor. **Siguen abiertos `qpdf` y `tesseract`** → C13 |
| **C4** | Las **20 herramientas de `video-audio-mcp` sin ejecutar**; si Claude Code emite `notifications/roots/list_changed`; si expone **recursos y prompts** al modelo; si las herramientas llegan **diferidas** | 🔴 `mcp-cabos-sueltos.md` §7 · **M1 sin lanzar** |
| **C5** | Carrera de **symlinks en Linux** contra `servers/filesystem`; punto de cruce exacto `inspect` vs staging (aquí ~90–100 MB) | 🔴 ídem · **M1 sin lanzar** |
...
| **C14** | **Ampliar el vocabulario de firmas del verificador** (hoy 24 nombres): el punto 1 del contrato solo fue evaluable en el **12 %** de los destinos de una muestra de 498 aristas | 🔴 **NUEVO** · ídem §11.3 |
...
| **C16** | **El 54,78 % de aristas indeterminadas.** Exige un corpus de 445 formatos que ningún motor local escribe (vía: corpus FATE de ffmpeg, ~1 GB). Es lo único que convierte el escenario central (48,6 %) en un número medido | 🔴 **NUEVO** · ídem §7, §11.1 |
...
| **C24** | **Aislar por qué el Tesseract externo falla en `d3` devolviendo 0 bytes y el embebido en Ghostscript falla ALUCINANDO al 165,8 %.** Mismo motor nominal, **modos de fallo opuestos según el envoltorio**: la diferencia tiene que estar en el preprocesado. **Es material directo para B7** (la heurística de degradación severa): un motor que devuelve 0 bytes y otro que devuelve más texto que la referencia son **la misma señal vista desde dos lados** | 🔴 **NUEVO** · `invocacion-aristas.md` §9, §11.7 |
...
| **C26** | **Que el lock de GPU sea de MÁQUINA y no de repositorio** (fichero en `%TEMP%` o mutex con nombre). Hoy `bench/lib/harness.sh` usa un lock dentro de `bench/` y **no ve otra sesión en el mismo equipo** | 🔴 **NUEVO** · `ppp-y-normalizacion.md` §1.3 |
...
| **G2 · Motores restantes** | B3 + B4 + B5, en ese orden y con timeout duro. **Un solo agente para los tres**, porque comparten el lock y el riesgo de tumbar CUDA | `bench/motores-restantes.md`, `bench/salidas-motores-restantes/` | **GPU (lock)** | 🔴 **sin lanzar** |
| **M1 · Cabos MCP** | C4 + C5 | `bench/mcp-cabos-2.md`, `bench/salidas-mcp-cabos-2/` | CPU | 🔴 **sin lanzar** |
...
| # | Trabajo | Recurso |
|---|---|---|
| **B13** | **Caracterizar el `k` de cada motor sobre más de un documento.** Hoy los siete óptimos salen de `escaneado_d4`: *que haya* un `k` por motor está apoyado, **su valor no**. Sin esto no se puede cablear en el adaptador. **Es el pendiente de más valor que abre la tanda** | GPU |
...
**Prioridad normal:** B6 (NVENC en lote), B7 y B8 (heurística de degradación —que ahora tiene **una señal más**, C24—, y R1 fuera del caso simple), B12, B14, B15, B16, C6, C7, C14, C16, C18, C22–C26. **Sin lanzar todavía: G2 (B3+B4+B5) y M1 (C4+C5).**

### Contención, en una frase

> **Nunca dos agentes de GPU a la vez** (lock de proyecto — **y ojo: no protege de otras sesiones de la máquina**). **Nadie escribe en los maestros salvo el agente de consolidación.** **Un fichero de informe por agente**, sin excepciones.
...
REGLA DE PPP — NO HAY UNA GLOBAL: HAY UNA POR MOTOR (ppp-y-normalizacion.md §2, 14:00)
  ...
  REGLA VIGENTE — la elige el ADAPTADOR DEL MOTOR, no el orquestador:
    ...
    ppp_ocr     = min(max(ppp_nativos, 100), ppp_nativos * 1.25) * k_motor
      k: PaddleOCR v6 medium 1,25 · RapidOCR v6 small+R6 1,00 · Docling+R6 1,00
         EasyOCR 1,00 · Tesseract 1,50 [n=1, PENDIENTE]
...
EL LOCK DE GPU ES DE PROYECTO, NO DE MAQUINA (ppp-y-normalizacion.md §1.3):
  gpu_acquire/gpu_release usan un fichero DENTRO de bench/. Excluye a otros agentes de
  FileX y NO VE NADA MAS. Una sesion de Claude en D:\Work\research\ASR ocupando
  11 754 de 12 288 MiB dejo una tanda 12 MINUTOS sin procesar una sola imagen.
  Si la GPU va lenta y el lock esta libre, MIRA LOS PID antes de culpar al arnes.
