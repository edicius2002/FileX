...
| 03/09 | **`bench/aristas-documentales-cierre.md`** (worker7, carril `filex-aristas-doc`) | **`fx.destinos('csv')` pasa de `[]` a 22 destinos.** La causa: nadie había añadido las tuplas `(origen, destino)` a `_DECLARADAS` en `LibreOfficeEnContenedor` ni `PandocEnContenedor` (`filex/motor_contenedor.py`), así que 15 aristas ya MEDIDAS (`ok`/`ok_parcial`, sondeo sellado del 02/09 por worker2) nunca entraban al grafo. Añadidas 8 en LibreOffice y 7 en Pandoc — **8, no 7**: el ENCARGO listaba 7 explícitas para LibreOffice y omitía `svg>pdf` por un error de transcripción, contradiciendo su propia aritmética ("quince de diecisiete") y su propio §3 (solo dos exclusiones: `pptx>png` y `svg>png`); se sumó la octava. Añadidos los 5 formatos que faltaban en `formatos.py` (`xlsx`, `ods`, `pptx`, `odp`, `tex`, los cinco `texto=True`). Tocar `_DECLARADAS` caducó la huella `motor` de las dos clases (trampa 32/61): las 55 aristas `REAL` resultantes (24+31) se RESONDEARON de verdad con `FileX.convertir()` real vía un arnés propio (no se copió el JSON con la huella cambiada), 0 de 55 con `rc≠0` o `contrato=fallo`. Efecto colateral MEDIDO y no arreglado (fuera de los 4 ficheros del encargo): `svg→pdf` ya no rasteriza y por eso `pruebas/test_hito4.py::Cobertura::test_el_aviso_de_rasterizacion_viaja_al_modelo` falla — su único ejemplo en vivo de "el único camino rasteriza" desapareció del grafo real; un barrido no encontró NINGÚN otro par (origen, destino) con ese mismo defecto hoy |
