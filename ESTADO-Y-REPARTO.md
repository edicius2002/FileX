...
>
> **Una fila = un identificador = exactamente un emoji de estado, y va en la última columna.** Verificado a máquina: **101 filas, 101 emojis.**
>
> | Emoji | Significa | Cuántos hoy |
> |---|---|---|
> | 🟢 | **CERRADO** con informe que lo prueba | **58** |
...
| **C27** | **Subir G6 de `aviso` a `fallo`** — hoy está calibrada sobre **22 casos de un solo motor**. Exige medirla con más motores y comprobar que no marca conversiones legítimas entre formatos equivalentes (`png` → `apng`, `mkv` → `mka`). **RESPONDIDO el 28/08 por V y la respuesta es NO, con la lista** (`bench/contrato-familia-resvg.md`): los dos casos que se temían **no pueden disparar G6** —ya están en `EXT_A_FIRMAS` y G6 exige que el destino no esté—; el riesgo real son **4 falsos positivos** (`vda → icb/vst/tga/vid`, alias de TGA); y **no aparece un segundo motor: 32 de 32 en ImageMagick, 0 de 41** en ffmpeg, Ghostscript, vips, soffice, inkscape y pandoc. **Sigue en `aviso`, ahora por decisión medida y no por falta de datos** | 🔴 **ABIERTO por decisión** · `contrato-familia-resvg.md` |
| **C28** | **Los 86 destinos indeterminados del censo de firmas**: 79 que ningún motor de esta máquina escribe y 7 donde la muestra describe al escritor y no al formato. **CERRADO el 28/08 por F2 — y los 86 no eran «79 + 7»: son 56 + 17 + 13**, y la tercera clase no estaba en el pendiente. De los 17 «banner del escritor», **0 tienen segundo escritor** entre los 20 adaptadores: **el remedio propuesto no tenía ingredientes**. Los cierra el prefijo, y **tres ya tenían su marcador dentro de la sonda** —faltaba la fila que lo acepta—: **17 de 17 sin escribir una muestra**. De los 56, el `rc` dice cuál de cinco remedios aplica: **FATE cierra 15**, ocho **no son formatos** y **21 se cierran con una invocación mejor**. Trampas 72 y 73 | 🟢 **CERRADO** · `firmas-cierre.md` |
...
| **C33** | **YA ESTABA CERRADO CUANDO SE ESCRIBIÓ ESTA FILA, y nadie lo comprobó — VERIFICADO el 27/08 sobre el código.** El diff de W9 se aplicó en el commit `3707751`, cuyo título lo dice: *«Hitos 3 y 4, y W9 cerrado dentro del propio núcleo de FileX»*. `nombre_seguro` se llama hoy en `filex/confinamiento.py:119` (lectura) y `filex/nucleo.py:192` (escritura). ~~**Es lo más urgente de esta sección**~~ — lo fue durante cuatro días **sobre un fallo que no existía**. La fila se copió del informe de K3 sin mirar el árbol: **un inventario que se escribe desde los informes y no desde el código envía agentes a arreglar lo ya arreglado** | 🟢 **CERRADO** · commit `3707751` |
...
| **C38** | **El lock de GPU solo existe en shell.** Ningún `.py` del repositorio llama a `gpu_acquire` (**0 de 15** arneses de Python que invocan `nvidia-smi`). Hoy se salva porque casi todos se lanzan desde un `.sh` que sí lo toma — **salvo `bench/scripts/whisper_precision.py` y `bench/scripts/gpuwatch.py`, que no los lanza ningún `.sh`.** En el paquete de producción no hay lock ninguno (→ N7). **⚠ EL PRESUPUESTO ESTÁ REFUTADO — MEDIDO el 28/08 por P** (`bench/cerrojo-unico.md`): `cerrojo-de-maquina.md` §12 lo llamaba *«una mudanza, no un diseño»* y **no lo es**. `bench/lib/harness.sh:173` toma el lock con `set -o noclobber` —creación exclusiva de fichero—, **incompatible con el candado de rango de bytes** de `filex/cerrojo.py`: un arnés de Python y un script de shell **no se excluirían entre sí**, que es lo contrario de lo que un lock debe hacer. **51 ficheros de `bench/` referencian ese harness** | 🔴 **ABIERTO, y más caro de lo que se creía** · `cerrojo-unico.md` |
...
| **N16** | **El punto ciego de A7**: por debajo de **48 kb/s** Opus rellena el canal mudo y la regla no dispara. **CERRADO el 28/08 por Y como MEDICIÓN, sin tocar `verificador.py`: la señal existe y no es la que proponía el pendiente.** La ventaja cruzada **no separa** (hueco −0,7983); **`corr(Rsal, Rent)` a secas sí, en las nueve tasas**, con meseta de umbral 0,008–0,13 donde atrapa **27 de 27 con 0 falsos de 45** (A7 hoy: 9 de 27). Cuesta **183,1 ms** donde A7 ya gasta 364,0: **si sustituye a los dos `astats`, es la mitad.** Falta aplicarlo, que es de otro reparto | 🟢 **CERRADO (medido; aplicar es N18)** · `ventana-antes-del-move.md` |
| **N17** | **`gpu-fase2.md` no es auditable:** sus salidas de texto de OCR ya no existen, así que sus cifras no se pueden recalcular con la métrica canónica. Es el precio de borrar binarios **sin guardar el texto**, y afecta a la regla §6 de `CLAUDE.md`: para OCR, el texto no es una salida binaria regenerable | 🔴 **NUEVO** · `metrica-ocr.md` §3 |
| **N18** | **Aplicar la señal de N16 a `verificador.py`**: `corr(Rsal, Rent)` atrapa **27 de 27 con 0 falsos** donde A7 atrapa 9, y **por la mitad de coste**. Y con ella habría que revisar si los dos `astats` siguen haciendo falta | 🔴 **NUEVO** · `ventana-antes-del-move.md` §8bis |
| **N19** | **`DirectorioDeTrabajo.recoger` sigue siendo pública y pisa en silencio.** Y lo dejó señalado y no lo tocó: es el mismo agujero que acaba de cerrarse en el `move`, un nivel más abajo | 🔴 **NUEVO** · ídem §6 |
| **N20** | **Un destino que es un DIRECTORIO se rechaza con el motivo equivocado.** FileX se niega, que es lo importante, pero el mensaje miente sobre por qué | 🔴 **NUEVO** · ídem §6 |
| **N21** | **El criterio del hito 6 NO es alcanzable, y falla la FORMA.** Supone que el pico de VRAM es propiedad de *qué modelos hay cargados*; es del **documento mayor y del camino**. Con EasyOCR, **un** modelo, sin NVENC, un A4 a 300 ppp da **13 094 MiB sobre una tarjeta de 12 288**. **Hay reescritura propuesta**, en tres condiciones y con las rectas dentro (1 080 y 719 MiB/Mpx). Es la trampa 68 | 🔴 **NUEVO** · `ocr-produccion-sidecar.md` §6.3 |
| **N22** | **`.pdb` lo escriben DOS motores y son dos formatos distintos** —el de GraphicsMagick es una imagen de Palm Pilot y el de Calibre un libro—, así que es otra colisión de extensión como `.x` y `.mat`. Es el único de los 11 `sin_vocabulario` que queda que parece deuda real: los otros nueve son **pseudoformatos** (`null`, `info`, `preview`, `histogram`, `vid`) o lenguajes de guión | 🔴 **NUEVO** · `firmas-cierre.md` |
| **N23** | **`bench/scripts/ocr_motor.py` sigue sin los dos testigos de ruido**, así que sus milisegundos valen solo como cifra relativa. Lo declara su propio autor | 🔴 **NUEVO** · `ocr-produccion-sidecar.md` §7.3 |
...
| ~~**A5**~~ | ~~**Ejecutar el commit.** Siete agentes sin versionar~~ — **HECHO**: `1fb5024` el 22/08 y `69f08df` el 27/08 | — |
