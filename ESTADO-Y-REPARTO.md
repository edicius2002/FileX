...
> |---|---|---|
> | 🟢 | **CERRADO** con informe que lo prueba | **74** |
> | 🟡 | **EN CURSO o PARCIAL** (una mitad cerrada, o hay un agente dentro) | **10** |
> | 🔴 | **ABIERTO** | **21** |
...
> ```
>
> Salida esperada hoy: `6 ⚫ · 21 🔴 · 10 🟡 · 74 🟢` sobre **111** filas *(verificado a máquina el 01/09, tras el saneo de veracidad de la ronda 3 — ver `bench/saneo-inventario.md` — y desde hoy lo verifica también `ci/integridad.py` en cada PR, que es lo que impide que esta línea vuelva a quedarse tres días diciendo otra cosa)*. *(23/08: `5 ⚫ · 44 🔴 · 6 🟡 · 32 🟢` sobre 87; 27/08: `6 · 40 · 5 · 36`; 28/08 tras la ronda 1: `6 · 42 · 5 · 39`; 29/08 con C39: `6 · 29 · 3 · 69` sobre 107; 01/09 antes del saneo: `6 · 24 · 8 · 73` sobre 111.)* **El saneo de la ronda 3 no movió filas por trabajo nuevo: movió DOS filas porque su propio color contradecía su propia evidencia** — `C28` estaba 🟢 y su informe fuente (`firmas-cierre.md` §8) declara sin resolver justo lo que la fila daba por cerrado; `C32` pedía arbitrar una contradicción que `C31`/`C37` ya habían arbitrado sin que nadie cerrara esta fila. Sube el neto de 🟡 en +2 y baja 🔴 en −2 con 🟢 sin cambio neto. **Esta línea estuvo TRES DÍAS diciendo 95 filas mientras la leyenda de arriba decía 107: es la trampa 44 dentro del documento que gobierna el reparto — un recuento honesto con una nota falsa al lado.)* **La ronda 1 cerró tres y abrió cinco; la ronda 2 cerró dos y medio cerró dos. Cerrar bien un pendiente casi siempre destapa los que tapaba — pero la proporción mejora cuando el encargo trae el mecanismo hecho: N10 se cerró rápido porque N1 ya había construido `filex/cerrojo.py`.**
>
...
| **C42** | **La lista de módulos aptos para la CI estaba medida en la plataforma equivocada — CERRADO EN PARTE el 01/09 por el master (commit `0999538`), enunciado desactualizado hasta hoy.** `ci/linux-apto.json` ya no dice «11 aptos, 262 pruebas» de WSL2/DrvFs/3.14: quedó remedido **en el runner de verdad** (`ubuntu-latest`, Python 3.11.16, ejecución `33538561732`) y da **7 aptos, 198 pruebas, 3,4 s** — de las dos listas (11 y 7) **solo coinciden cinco módulos**, no una contiene a la otra. **Y el `continue-on-error: true` A NIVEL DE TRABAJO ya se quitó** del job `linux` de `.github/workflows/suite.yml`: verificado en el fichero hoy, solo queda en el job `deriva` (el que remide por cron/manual para detectar deriva, donde SÍ debe informar sin bloquear). El criterio de cierre que la propia fila pedía («quitar ese `continue-on-error`») **ya se ejecutó**. **Lo que sigue abierto de verdad, y es lo que `PENDIENTE.md` §3 ya declara correctamente:** de los 17 módulos, **10 no corren en el runner — 1 cuelga (`test_cancelacion`) y 9 fallan** por pedir tarjeta GPU, motores nativos o NTFS que Linux no tiene. Ese residuo no se cierra con más CI: es la brecha estructural que documenta `CONTRIBUTING.md` §1 | 🔴 **enunciado corregido, residuo real 10/17** · trampa 104, commit `0999538` |
...
| **N27** | **La recta de RapidOCR SUBESTIMA 339 MiB en el tramo medio** (r²=0,7581, residuo negativo en 2 de 5 puntos). No rompe nada porque el margen de 500 lo tapa por 161 — **y eso convierte el margen en la pieza que sostiene el modelo**: bajarlo deja una celda en descubierto. Trampa 85 | 🔴 **NUEVO** · ídem §8.3 |
...
| Ruta | Para qué |
|---|---|
| `CLAUDE.md` | Las reglas de trabajo, las **106** trampas ya pagadas y las reglas de diseño no negociables. ~~24~~ — **la cifra llevaba setenta y ocho trampas desfasada**, que es el mismo defecto que esta sección viene a arreglar |
