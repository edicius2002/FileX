...
| 01/09 | **`bench/gotenberg-y-mcp.md`** (worker2) | **`C35` y `C36`.** Gotenberg, que es lo que el título del hito 5 pedía, y los ocho pendientes de `hito4-mcp.md` §13 —**de los que dos ya estaban cerrados** |
...
| **C40** | **Diez binarios versionados fuera de LFS, 17 MB** — salidas de competidores y referencias MCP, que son justo lo que la regla §6 dice que no se versiona: *«no versiones salidas binarias regenerables»*. **Contados, no estimados**, por `ci/integridad.py` y congelados en `ci/heredado.json`: hoy no rompen la CI, pero **uno nuevo sí**. Cerrarlo es borrarlos y dejar el `MANIFIESTO.md` con la orden que los reproduce | 🔴 **NUEVO** · `ci/heredado.json` |
...
| **worker2** (carril CPU/Docker) | **C35 + C36** | `bench/gotenberg-y-mcp.md` | 🟢 **CERRADO.** Y una autocorrección: de los ocho pendientes de `hito4-mcp.md` §13, **dos ya estaban cerrados** |
...
| Ruta | Para qué |
|---|---|
| `CLAUDE.md` | Las reglas de trabajo, las **103** trampas ya pagadas y las reglas de diseño no negociables. ~~24~~ — **la cifra llevaba setenta y ocho trampas desfasada**, que es el mismo defecto que esta sección viene a arreglar |
