...
>
> **Una fila = un identificador = exactamente un emoji de estado, y va en la última columna.** Verificado a máquina: **92 filas, 92 emojis.**
>
> | Emoji | Significa | Cuántos hoy |
> |---|---|---|
> | 🟢 | **CERRADO** con informe que lo prueba | **39** |
...
> ```
>
> Salida esperada hoy: `6 ⚫ · 42 🔴 · 5 🟡 · 39 🟢` sobre **92** filas. *(23/08: `5 ⚫ · 44 🔴 · 6 🟡 · 32 🟢` sobre 87; 27/08: `6 · 40 · 5 · 36`.)* **La ronda 1 cerró tres (C34, N1, N6) y abrió cinco (N10-N14): cerrar bien un pendiente casi siempre destapa los que tapaba.**
>
...
| **N10** | **La cancelación es DE PROCESO**, como lo era el cerrojo de destino: el registro `{ident → Popen}` vive en la memoria de un `filex`. La respuesta lo dice (`motor_detenido: false`) en vez de fingirlo. Cerrarlo pide un canal con nombre o un fichero de mando por trabajo — **y ahora hay primitivo: `filex/cerrojo.py`** | 🔴 **NUEVO** · `cancelacion-y-servicio.md` §4.1 |
| **N11** | **`olvidar_hilo()` es una DISCIPLINA que hay que recordar**, y este repositorio evita justamente eso en las invocaciones: los `ident` de hilo se reciclan y quien añada una tercera clase de trabajo tiene que llamarla. Lo limpio es un envoltorio `with invocacion.hilo_de(t):` | 🔴 **NUEVO** · ídem §4.3 |
...
| **N13** | **POSIX se queda a medias en las dos mitades**: sin detección equivalente a `os.replace` y sin barrido del candado. Emparejado con **N4**, que dice lo mismo del watcher | 🔴 **NUEVO** · `cerrojo-de-maquina.md` §6.5, §6.6 |
| **N14** | **Un `taskkill /F` deja sin borrar un desechable de R18** por conversión en vuelo. No era el encargo de nadie, pero está medido | 🔴 **NUEVO** · `cerrojo-de-maquina.md` §4.1 |
