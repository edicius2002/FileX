...
| **N32** | **El suelo temporal de `N9` cierra el oráculo A LA MEDIANA y NO en la cola, y la frase que justificaba su valor era falsa contra su propio artefacto — MEDIDO el 03/09 al verificar la ronda 10** (`bench/oraculo-y-gotenberg.md` §1.4). `PISO_TEMPORAL_S = 300 µs` lleva el ratio `no_existe/prohibido` de **17,53× a 1,00× a la mediana**, que es el resultado y es bueno. **Pero a p90 el camino ecualizado sigue en 582,19 µs frente a 308,60 del denegado: 1,88×**, y un atacante que promedie muchas muestras lo ve. El informe afirmaba que el suelo se fijó *«con margen sobre el p90 de `no_existe` sin ecualizar (364,65 µs)»* y **300 queda por DEBAJO de 364,65**; el comentario de `filex/confinamiento.py` lo justificaba con otras cifras —161,95 y 271,19 µs— que **no están en el `resultado.json` versionado**. Dos tandas, y la que sostenía la conclusión no se guardó: **trampa 55 dentro de la 44**. Las dos frases quedan corregidas; **el valor NO se tocó a ojo**. **Lo que hay que decidir, midiendo:** subir el suelo por encima del p90 cierra la cola **y sube el coste del rechazo**, que es exactamente el amplificador de DoS de la trampa 28 — el mismo conflicto de `N9`, un nivel más abajo. Y hay una salida que nadie ha medido: **un suelo por OPERACIÓN en vez de por llamada** cerraría de paso el residuo de 2,11× que §1.5 declara | 🔴 **NUEVO** · `bench/oraculo-y-gotenberg.md` §1.4-1.5 |
