...
- **`inspect` es la excepción a R8 y a R18: se queda en proceso y en sitio.** Lee cabeceras, no entrega la ruta a ningún motor externo y no escribe nada, así que no hay staging que justificar ni censo que hacer. **Con número:** el `inspect` en proceso cuesta **0,04–0,06 ms**; el staging que R8 le impondría, de **1,7 ms (1 MB) a 166 ms (256 MB)** — de 30× a más de 3.000× la operación **a cambio de cero seguridad**. El cruce no es una constante: **`cruce_MB ≈ ffprobe_ms × copia_MBps / 1000`**, que con `ffprobe` ≈ 57 ms da **~70 MB con el disco contendido y ~95 MB holgado**.
